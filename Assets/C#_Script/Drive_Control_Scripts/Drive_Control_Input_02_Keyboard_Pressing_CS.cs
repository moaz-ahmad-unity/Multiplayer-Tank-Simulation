...
namespace ChobiAssets.PTM
{
    ...
    public class Drive_Control_Input_02_Keyboard_Pressing_CS : Drive_Control_Input_01_Keyboard_Stepwise_CS, IPunObservable
    {
        ...
        public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
        {
            if (stream.IsWriting)
            {
                stream.SendNext(controlScript.Stop_Flag);
                stream.SendNext(controlScript.L_Input_Rate);
                stream.SendNext(controlScript.R_Input_Rate);
                stream.SendNext(controlScript.Turn_Brake_Rate);
                stream.SendNext(controlScript.Pivot_Turn_Flag);

            }
            else 
            {
                controlScript.Stop_Flag = (bool)stream.ReceiveNext();
                controlScript.L_Input_Rate = (float)stream.ReceiveNext();
                controlScript.R_Input_Rate = (float)stream.ReceiveNext();
                controlScript.Turn_Brake_Rate = (float)stream.ReceiveNext();
                controlScript.Pivot_Turn_Flag = (bool)stream.ReceiveNext();
            }
        }
    }
