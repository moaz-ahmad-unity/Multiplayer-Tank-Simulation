...
namespace ChobiAssets.PTM
{
    ...
    public class Drive_Wheel_Parent_CS : MonoBehaviour,IPunObservable
    {
        /*
		...
		*/
        ...
        public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
        {
            if (stream.IsWriting)
            {
                stream.SendNext(Max_Angular_Velocity);
                stream.SendNext(Left_Angular_Drag);
                stream.SendNext(Right_Angular_Drag);
                stream.SendNext(Left_Torque);
                stream.SendNext(Right_Torque);
            }
            else 
            {
                Max_Angular_Velocity = (float)stream.ReceiveNext();
                Left_Angular_Drag = (float)stream.ReceiveNext();
                Right_Angular_Drag = (float)stream.ReceiveNext();
                Left_Torque = (float)stream.ReceiveNext();
                Right_Torque = (float)stream.ReceiveNext();
            }
        }
    }
