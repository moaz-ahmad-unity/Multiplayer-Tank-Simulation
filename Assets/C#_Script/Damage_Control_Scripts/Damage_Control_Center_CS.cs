...
namespace ChobiAssets.PTM
{
	...
	public class Damage_Control_Center_CS : MonoBehaviour,IPunObservable
	{
		/* 
		...
		*/
        ...
        bool MainBody_Damaged(float damage)
        {
            ...
            if (MainBody_HP <= 0)
            {
                ...
                {
                    parentPhotonView.RPC("MainBody_Destroyed",RpcTarget.AllViaServer);
                }
                return true;
            }
            return false;
        ...
        bool Turret_Damaged(float damage, int index)
        {
            ...
            if (Turret_Props[index].hitPoints <= 0)
            {
                ...
                else
                {
                    if (parentPhotonView != null && parentPhotonView.IsMine)
                    {
                        parentPhotonView.RPC("Turret_Destroyed", RpcTarget.AllViaServer,index);
                    }
            ...
            switch (index)
            {
                case 0: // Left track
                    ...
                    if (Left_Track_HP <= 0)
                    {
                        if (parentPhotonView != null && parentPhotonView.IsMine)
                        {
                            parentPhotonView.RPC("Track_Destroyed", RpcTarget.AllViaServer, true);
                        }
                ...
                case 1: // Right track
                    ...
                    if (Right_Track_HP <= 0)
                    {
                        if (parentPhotonView != null && parentPhotonView.IsMine)
                        {
                            parentPhotonView.RPC("Track_Destroyed", RpcTarget.AllViaServer, false);
                        }
        ...
        void MainBody_Destroyed()
        {
            ...
            for (int i = 0; i < Turret_Props.Length; i++)
            {
                if (parentPhotonView != null && parentPhotonView.IsMine)
                {
                    parentPhotonView.RPC("Turret_Destroyed", RpcTarget.AllViaServer, i);
                }
                //Turret_Destroyed(i);
            }
        ...
        public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
        {
            if (stream.IsWriting)
            {
                stream.SendNext(MainBody_HP);
                stream.SendNext(Turret_Props[0].hitPoints);
                stream.SendNext(Left_Track_HP);
                stream.SendNext(Right_Track_HP);
            }
            else
            {
                MainBody_HP = (float)stream.ReceiveNext();
                Turret_Props[0].hitPoints = (float)stream.ReceiveNext();
                Left_Track_HP = (float)stream.ReceiveNext();
                Right_Track_HP = (float)stream.ReceiveNext();
            }
        }
    }
